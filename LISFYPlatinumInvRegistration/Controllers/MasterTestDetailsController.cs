...
namespace LISFYPlatinumInvRegistration.Controllers
{
    ...
    public class MasterTestDetailsController : Controller
    {
        ...
        public JsonResult LookUpEntry(int SrchCode)
        {
            ...
            if (SrchCode == 0)
            {
                try
                {
                    ...
                    var tstdls = db.Stproc_TEST_MASTER_DTLS().ToList();
                    return Json(new { Result = "OK", Records = tstdls }, JsonRequestBehavior.AllowGet);
                }
                catch (Exception exe)
                {
            ...
            else
            {
                ...
                {
                    tstream.WriteLine("");
                    tstream.WriteLine(DateTime.Now + "errorss last -->");
                    tstream.WriteLine("");
                    tstream.Flush();
                    tstream.Close();
                }
        ...
        //    return Json(true);
        //}DeleteDoctor
        public JsonResult DeleteDoctor(double dctrKey, string dctrName)
        {
            int Drcount = 0;
            ...
            cmd = new SqlCommand("SELECT Count(Inv_no) FROM INVOICE_MST WHERE INV_DRID=" + dctrName + "", sqlcon);
            try { Drcount = Convert.ToInt32(cmd.ExecuteScalar()); } catch { }
            if (Drcount > 0)
            {
                return Json(new { Result = "Transaction Found,You Can't Delete" });
                //MessageBox.Show("Transaction Found,You Can't Delete....", cPublic.ProjectName);
                //return;
            }
            cmd = new SqlCommand("SELECT Count(BknKey) FROM Booking_Details WHERE Bkn_DoctrID=" + dctrKey + "", sqlcon);
            try { Drcount = Convert.ToInt32(cmd.ExecuteScalar()); } catch { }
            if (Drcount > 0)
            {
                //MessageBox.Show("Transaction Found,You Can't Delete....", cPublic.ProjectName);
            ...
            sqlcmd2.CommandText = "DELETE FROM AccountHeads_Mst WHERE AhMst_Key=" + dctrKey + "";
            sqlcmd2.Parameters.Clear();
            sqlcmd2.ExecuteNonQuery();

            string usrLogData = "";
            usrLogData = "DOCTOR DATA DELETED -NAME : " + dctrName;
            //cPublic.UserTrackInsert(usrLogData, "DOCTOR", 34);
            ...
            try
            {
                ...
                cmd.Parameters.AddWithValue("@Log_TransNo", dctrKey.ToString());
                cmd.Parameters.AddWithValue("@Log_DateTime", DateTime.Now);
                cmd.Parameters.AddWithValue("@Log_Desc", dctrName + "(" + dctrKey + ")" + " Doctor  Removed.... ");
                cmd.Parameters.AddWithValue("@Log_Form", "Doctor");
        ...
        //GpTstDls
        public JsonResult GpTstDlsSrch(string term,int tstTypeFlag,string dept,int? deptId)
        {
            //if (_TestTypeId == -2 || _TestTypeId == -12 || _TestTypeId == -15 || _TestTypeId == -24)
            if(tstTypeFlag==0)
            {
                var gpTst = db.Test_Mst.Where(tst => tst.TstMst_name.Contains(term) && tst.TstMst_DeptId== deptId).Select(t => new { t.TstMst_Key, t.TstMst_name }).ToList();
                return Json(new { Result = "OK", Record = gpTst }, JsonRequestBehavior.AllowGet);
            }
            else if(tstTypeFlag == 1)
            {
                var gpTst = db.Test_Mst.Where(tst => tst.TstMst_name.Contains(term)).Select(t => new { t.TstMst_Key, t.TstMst_name }).ToList();
                return Json(new { Result = "OK", Record = gpTst }, JsonRequestBehavior.AllowGet);
            }
            else
            {
        ...
        public JsonResult DoctorDls(string dctrName, int dctrKey)
        {
            try
            {
                ...
                if (dtpDob == "")
                {
                    //dtpDob = "01/01/0001 00:00:00 AM";
                    dtpDob = (new DateTime()).ToString();
                }
                if (dtpWedding == "")
                {
                    //dtpDob = "01/01/0001 00:00:00 AM";01/01/0001 00:00:00
                    dtpWedding = (new DateTime()).ToString();
                }
                //************************************************
                txtGroup = dt0.Rows[0]["GroupName"].ToString();
                txtGroup = dt0.Rows[0]["GROUPCODE"].ToString();
                nGROUPLEVEL = dt0.Rows[0]["GROUPLEVEL"].ToString();
                double txtCatKey = 0, txtDocSplzKey = 0, txtDocHosKey = 0;
                nLEVEL1 = dt0.Rows[0]["LEVEL1"].ToString();
                if (txtCatgKey == "")
                {
                    txtCatKey = Double.Parse("0");
                }
                else
                {
                    //txtCatKey=Convert.ToDouble(txtCatgKey);
                    txtCatKey = double.Parse(txtCatgKey, System.Globalization.CultureInfo.InvariantCulture);
                }

                if (txtDocSplzdKey == "")
                {
                    txtDocSplzKey = Double.Parse("0");
                }
                else
                {
                    //txtDocSplzKey = Convert.ToDouble(txtCatgKey);
                    txtDocSplzKey = double.Parse(txtDocSplzdKey, System.Globalization.CultureInfo.InvariantCulture);
                }

                if (txtDocHospKey == "")
                {
                    txtDocHosKey = Double.Parse("0");
                }
                else
                {
                    //txtDocHosKey = Convert.ToDouble(txtCatgKey);
                    txtDocHosKey = double.Parse(txtDocHospKey, System.Globalization.CultureInfo.InvariantCulture);
                }
                //*********************************************************************************
                doctorDtls dctrs = new doctorDtls()
                {
                    ...
                    CatgKey = Convert.ToDouble(txtCatKey),
                    DocSpecialised = txtDocSpecialised,
                    DocSplsdKey = Convert.ToDouble(txtDocSplzKey),
                    DocEdu = txtDocEdu,
                    DocHosp = txtDocHosp,
                    dctr_hospaddr = txtHospAddress,
                    DocHospKey = Convert.ToDouble(txtDocHosKey),
