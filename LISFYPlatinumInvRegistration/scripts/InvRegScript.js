    ...
    if (keycode == '13') {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.Result === "OK") {
                    if ((data.invLst).length != 0) {
                        if (data.Records == parseInt(0)) {
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    if (data.Result === "OK") {
                                        ...
                                        if (data.Record[0].PendingAmt != 0) {
                                            $("#PColl").text("P.Coll:" + data.Record[0].PendingAmt);
                                        }
                                        else {
                                            $("#PColl").text("");
                                        }
                                        ...
                                        if (data.Record[0].Inv_BalAmt != 0 && payMode == 'Cash') {
                                            $("#PayPend").text("Payment Pending!");
                                            //$('#PayPend').blink(100);
                                        }
                                        else {
                                            $("#PayPend").text("");
...
//rsltOnSlct rTrigger ResultOn
$("#rsltOnSlct").datepicker({
    dateFormat: 'dd/mm/yy',
    changeMonth: true,
    changeYear: true,
    yearRange: '-90:+80',
...
$("#rsltOnSlct").change(function () {
    var sampleOnSlct = $("#rsltOnSlct").val();
    var splitSmpl = sampleOnSlct.split("/");
    var mn = splitSmpl[1];
    var yr = splitSmpl[2];
    var dt = splitSmpl[0];
    var val = new Date(mn + "/" + dt + "/" + yr);
    var dt = new Date();
    var shortMonth = val.toLocaleString('en-us', { month: 'short' });
    let a = dt.toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true });
    if (val.getDate() < 10) {
        let smplOn = "0" + val.getDate() + "-" + shortMonth + "-" + val.getFullYear() + " " + a;
        document.querySelector("#ResultOn").value = smplOn;
    }
    else {
        let smplOn = val.getDate() + "-" + shortMonth + "-" + val.getFullYear() + " " + a;
        document.querySelector("#ResultOn").value = smplOn;
    }
});
$(".rTrigger").click(function () {
    $("#rsltOnSlct").datepicker("show");
});
//////////////////////////////////////////////////////////////////////////////////
$("#sampleOnSlct").datepicker({
    dateFormat: 'dd/mm/yy',
    changeMonth: true,
    changeYear: true,
    yearRange: '-90:+80',
...
$("#sampleOnSlct").change(function () {
    var sampleOnSlct = $("#sampleOnSlct").val();
    var splitSmpl = sampleOnSlct.split("/");
    var mn = splitSmpl[1];
    var yr = splitSmpl[2];
    var dt = splitSmpl[0];
...
$(".srchClose").on('click', function (event) {
    $("#PatDtl").val("");
})
...
$("#invPrint").on('click', function () {
    ...
    var cpyid = $("#cpyid").val();
    var yrid = $("#yrid").val();
    var x = $("#grandTotal").text();
    var netAmt = parseFloat(x.match(/[-+]?([0-9]*\.[0-9]+|[0-9]+)/));
    var PatPrntObj = {
        Inv_No: invNo,
        Inv_InsId: corpKey,
        Inv_CpyId: cpyid,
    ...
    $.ajax({
        url: "/InvoiceRegister/PatientPrint",
        type: "POST",
        data: JSON.stringify(PatPrntObj),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
            if (data.Result === "OK") {
                window.location = "/InvoiceRegister/PDFPatientPrint";
            }
            else {
...
$(".zoombtn").on('click', function (e) {
    var btnId = $(this).find(".fa-search").attr("id");
    var imgId = $(this).parent().find("img").attr("id");
    //var imgId = $(this).parentsUntil("#a").attr("id");
    //var imgIsd = $(this).find(".lazy").attr("id");
    var scanImg;
    if (btnId == "SrchScan1") {
        scanImg = 1;
    }
    else if (btnId == "SrchScan2") {
        scanImg = 2;
    }
    else if (btnId == "SrchScan3") {
        scanImg = 3;
    }
    else if (btnId == "SrchScan4") {
        scanImg = 4;
    }
    $.ajax({
        url: "/InvoiceRegister/ScanImage",
        type: "POST",
        data: JSON.stringify({ scanImage: scanImg }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (data) {
            if (data.Result === "OK") {
...
$("#patIdSrchBtn").on('click', function (event) {
    event.preventDefault();
    $.ajax({
        url: "/InvoiceRegister/SerchPatnt",
        type: "POST",
        data: {},
        contentType: "application/json; charset=utf-8",
        dataType: "json",
                                                                    ...
                                                                    if ($("#clinicId").val() == '' && $("#clinic").val() != '') {
                                                                        ...
                                                                        if ($("#collPersonId").val() == '' && $("#collPerson").val() != '') {
                                                                            ...
                                                                            if ($(testTableRows).find('#tsname').text() == "" && $(testTableRows).find('#tscode').text() == "" && !(testTableRows.length > 1)) {
                                                                                ...
                                                                                if ($("#DiscAmt").val() != 0) {
                                                                                    var disc = $("#DiscAmt").val();
                                                                                    var gm = $("#grandTotal").text().replace(/[^0-9]/gi, '');
                                                                                    var gndtotl = gm - disc;
                                                                                    document.getElementById("netamt").innerHTML = "&#8377;" + gndtotl + "&#8725;&#9592;";
...
$('#DiscAmt').keyup(function () {
    ...
    nScharge = parseFloat($("#SChrge").val());
    if (isNaN(nScharge)) {
        nScharge = 0;
    }
...
$("#Paytype").change(function () {
    ...
    if (pmode == "Credit") {
        ...
        var TotalBalance = parseInt((total - disc + SChrge) - PaidAmt);
        //$("#PaidAmt").val(0);
        if ($("#PaidAmt").val() != 0 && $("#balance").val() != 0) {
            return false;
        }
        else {
            $("#PaidAmt").val(0);
    ...
    if (moment(dateString, 'MM/DD/YYYY HH:MM tt', true).isValid()) {
        ...
        if (m < 10) {
            ...
            if (d.getDate() < 10) {
                ...
                let smplOn = "0" + date.getDate() + "-" + shortMonth + "-" + date.getFullYear() + " " + a;
                document.querySelector("#ResultOn").value = smplOn;
                ...
                var datetime = new Date();
                let a = datetime.toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true });
                let smplOn = date.getDate() + "-" + shortMonth + "-" + date.getFullYear() + " " + a;
                document.querySelector("#ResultOn").value = smplOn;
            }
        }
        else {
            if (d.getDate() < 10) {
                var datetime = new Date();
                let a = datetime.toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true });
                let rslOn = "0" + d.getDate() + "-" + shortMonth + "-" + d.getFullYear() + " " + a;
                document.querySelector("#ResultOn").value = rslOn;
            }
            else {
                var datetime = new Date();
                let a = datetime.toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true });
                let rslOn = d.getDate() + "-" + shortMonth + "-" + d.getFullYear() + " " + a;
                document.querySelector("#ResultOn").value = rslOn;
            }
        }
    }
...
$("#SampleOn").change(function (e) {
    ...
    if (moment(dateString, 'DD-MMM-YY HH:MM tt', true).isValid()) {
        return;
    }
    else {
        alert("Please enter the date in correct format!");
            ...
            if ($('#testId').val() == "") {
                ...
                if (lngth == 2 && (tstCodes == "" && tstname == "")) {
                    $.each(testSearch, function (index, test) {
                        ...
                        var rptday = testSearch.tst_RptDay;
                        var timeDays = testSearch.tst_RptTmeDays;
...
$('body').on('dblclick', '#tstList', function (evt) {
...
$(document).ready(function () {
    ...
    var m = date.getMonth() + 1;
    var mlength = m.toString.length;
    let a = date.toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true });
    if (date.getDate() < 10) {
        let requestdate = "0" + date.getDate() + "-" + shortMonth + "-" + date.getFullYear();
        ...
        let requestdate = date.getDate() + "-" + shortMonth + "-" + date.getFullYear();
        let smplOn = date.getDate() + "-" + shortMonth + "-" + date.getFullYear() + " " + a;
        document.querySelector("#SampleOn").value = smplOn;
        document.querySelector("#requestdate").value = requestdate;
        document.querySelector("#ResultOn").value = smplOn;
